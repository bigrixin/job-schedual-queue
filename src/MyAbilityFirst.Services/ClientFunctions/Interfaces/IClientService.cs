...
namespace MyAbilityFirst.Services.ClientFunctions
{
	public interface IClientService
	{
		...
		// Booking Services
		Booking CreateNewBooking(int clientID, int careWorkerID, Schedule schedule, string message, int? jobID);
		...
		List<JobViewModel> GetJobsListVM(int clientID);
		IEnumerable<SelectListItem> GetCarerInShortlist(int careWorkerID);
		void RejectCareWorker(int clientID, int careWorkerID, int jobID);
	}
}