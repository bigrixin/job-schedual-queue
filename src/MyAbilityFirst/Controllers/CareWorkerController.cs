...
namespace MyAbilityFirst.Controllers
{
	...
	public class CareWorkerController : Controller
	{
		...
		public ActionResult Bookings(int? page)
		{
			...
			ViewBag.CareWorkStatus = carer.Status.ToString();
		...
		[HttpGet, Route("careworker/jobs")]
		public ActionResult Jobs(int? page)
		{
			var careWorker = this.GetLoggedInUser() as CareWorker;
			ViewBag.CareWorkStatus = careWorker.Status.ToString();
			ViewBag.CareWorkerID = careWorker.ID;
			//get jobs by set range
			var jobsVM = this._careWorkerServices.GetJobsListViewModes(careWorker.ID);
			ViewBag.Total = jobsVM.ToArray().Length;
			int pageNumber = (page ?? 1);
			pageIndex = page.HasValue ? Convert.ToInt32(page) : 1;
			return View(jobsVM.ToPagedList(pageNumber, pageSize));
		}

		[HttpGet, Route("careworker/job/{jobID:int}")]
		public ActionResult Job(int jobID)
		{
			if (jobID == 0)
				return new HttpStatusCodeResult(HttpStatusCode.BadRequest);

			var vm = this._careWorkerServices.GetJobVM(jobID);
			return View(vm);
		}

		[HttpGet, Route("careworker/expressinteresting/{jobID:int}")]
		public ActionResult ExpressInteresting(int jobID)
		{
			var careWorker = this.GetLoggedInUser() as CareWorker;
			this._careWorkerServices.SetExpressInteresting(careWorker.ID, jobID);
