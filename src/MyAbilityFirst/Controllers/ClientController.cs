...
namespace MyAbilityFirst.Controllers
{
	...
	public class ClientController : Controller
	{
		...
		public ActionResult NewBooking(int? careWorkerID, int? jobID)
		{
			var client = this.GetLoggedInUser() as Client;

			var vm = new NewBookingViewModel();
			if ((careWorkerID != null) && (jobID != null))
			{
				var job = client.GetJob((int)jobID);
				if (job != null && (job.Status == JobStatus.Interested))
				{
					vm.Shortlist = this._clientServices.GetCarerInShortlist((int)careWorkerID);
					vm.Message = "Re: job #" + job.ID + " - '" + job.Title + "'" + "\n" + job.Description;
					vm.Start = job.ServiceAt.Value;
					vm.End = job.ServiceAt.Value.AddDays(1);
				}
				else
			...
			{
				vm.Shortlist = this._clientServices.GetShortlist(client.ID)
																				.Select(s => new SelectListItem
																				{
																					Value = s.CareWorkerID.ToString(),
																					Text = s.CareWorkerFirstName
																				});
				vm.Start = DateTime.Today;
				vm.End = DateTime.Today.AddDays(1);
			};
		...
		public ActionResult NewBooking(NewBookingViewModel vm)
		{
			if (ModelState.IsValid)
			{
				...
				var booking = this._clientServices.CreateNewBooking(clientID, careWorkerID, schedule, message, vm.JobID);
		...
		public ActionResult RejectCareWorker(int careWorkerID, int jobID)
		{
			var client = this.GetLoggedInUser() as Client;
			this._clientServices.RejectCareWorker(client.ID, careWorkerID, jobID);
